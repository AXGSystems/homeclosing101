      ...
      <div className={`flex items-center justify-center gap-6 flex-wrap px-4 transition-opacity duration-400 ${fading ? "opacity-0" : "opacity-100"}`}>
        {visible.map((s) => (
          ...
          >
            ...
            <img src={s.logo} alt={s.name} className="h-6 w-auto object-contain max-w-[100px]" loading="lazy" onError={(e) => { if (!e.currentTarget.src.endsWith('/logos/alta.svg')) e.currentTarget.src = '/logos/alta.svg'; }} />
          </a>
        ))}
      </div>
    </div>

...
function AchievementsSection() {
  ...
  return (
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 border-t border-gray-200">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-alta-navy mb-2">Your Achievements</h2>
        <p className="text-sm text-alta-gray">Track your HomeClosing101 learning journey. {unlockedCount} of {total} unlocked.</p>
      </div>

      {/* Progress bar */}
      <SectionGate page="/trivia" id="achievements" sub="progress">
      ...
      {/* Achievement grid */}
      <SectionGate page="/trivia" id="achievements" sub="grid">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {achievements.map((a) => (
          ...
          >
            <div
              className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3 ${a.unlocked ? '' : 'grayscale'}`}
              style={{ backgroundColor: a.unlocked ? `${a.color}15` : '#f3f4f6' }}
            >
              {a.unlocked ? (
                <svg className="w-8 h-8" style={{ color: a.color }} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
            ...
            {a.unlocked && (
              <div className="absolute top-3 right-3">
                <div className="w-6 h-6 rounded-full flex items-center justify-center" style={{ backgroundColor: a.color }}>
            ...
            <h3 className={`text-sm font-bold mb-1 ${a.unlocked ? 'text-alta-navy' : 'text-gray-400'}`}>{a.title}</h3>
            <p className={`text-xs leading-relaxed ${a.unlocked ? 'text-alta-gray' : 'text-gray-300'}`}>{a.description}</p>
      ...
      {unlockedCount < total && (
        <div className="mt-10 text-center p-6 bg-alta-light rounded-xl border border-gray-100">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <Link href="/first-time-buyers" className="text-sm font-medium text-alta-teal hover:underline">First-Time Buyers</Link>
            <span className="text-gray-300">|</span>
            <Link href="/mortgage-calculator" className="text-sm font-medium text-alta-teal hover:underline">Mortgage Calculator</Link>
            <span className="text-gray-300">|</span>
            <Link href="/glossary" className="text-sm font-medium text-alta-teal hover:underline">Glossary</Link>
          </div>
        </div>
      )}
    ...
    <>
      ...
      <div className="py-1.5 lg:py-2">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <SectionGate page="/trivia" id="sponsor-tip">
          ...
          {/* ══ JEOPARDY BOARD ══ */}
          {!gameOver && (
            <SectionGate page="/trivia" id="board">
